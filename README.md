...
flowchart TB
    subgraph CLIENT["📱 Client Layer"]
        direction TB
        GPS["🛰️ GPS Receiver<br/><i>lat, lon, speed, heading</i>"]
    ...
    subgraph SERVER["⚙️ FastAPI Server"]
        ...
        subgraph SERVICES["📦 Services Layer"]
            direction LR
            RISK["⚠️ Risk Service<br/><i>Hazard calculation</i>"]
            GEO["🗺️ Geo Service<br/><i>Distance, curves, grades</i>"]
    ...
    subgraph STORAGE["🗄️ Storage Layer"]
        ...
        SQLITE[("🗃️ SQLite<br/><i>rides, samples, debriefs</i>")]
    end
...
[![Complete Ride Session Flow](ride-session-flow.png)](ride-session-flow.png)

```mermaid
sequenceDiagram
    autonumber

    participant R as 🏍️ Rider Device
    participant A as ⚙️ API Server
    participant S as 📋 State Manager
    participant G as 🗺️ Geo Service
...
[![Risk Calculation Engine](risk-calculation-engine.png)](risk-calculation-engine.png)
...
flowchart TD
    ...
    subgraph GEOMETRY["📐 GEOMETRIC ANALYSIS"]
        ...
        GRADE["Calculate Grade %<br/><i>Δelevation / distance</i>"]
    end

    subgraph HAZARDS["⚡ HAZARD SCORING"]
        direction TB
        CH["🔄 CURVE HAZARD<br/>Weight: 45%<br/><code>1 / (radius × speed_factor)</code>"]
        GH["📈 GRADE HAZARD<br/>Weight: 35%<br/><code>|grade| / 10%</code>"]
    ...
    subgraph CALC["🧮 RISK CALCULATION"]
        FORMULA["<b>risk = 100 × clamp(</b><br/>0.45×curve +<br/>0.35×grade +<br/>0.15×surface +<br/>0.05×trend<br/><b>, 0, 1)</b>"]
    end
    ...
    classDef hazard fill:#fce4ec,stroke:#c2185b
    classDef calc fill:#f3e5f5,stroke:#7b1fa2
    classDef output fill:#e8f5e9,stroke:#2e7d32

    class INPUT input
    class ROUTE route
    class HAZARDS hazard
    class CALC calc
    class OUTPUT output
```
...
```
grade_pct = (Δelevation / horizontal_distance) × 100

grade_hazard = clamp(|grade_pct| / 10, 0, 1)
```
...
| Risk Engine | [Risk Calculation Deep Dive](#risk-calculation-deep-dive) | Telemetry → Risk flowchart |
